...
Chemical accuracy for H$_2$ VQE was achieved on both IBM Torino (0.22\kcalmol{} with TREX) and QI Tuna-9 (0.92\kcalmol{} with a hybrid post-selection + readout error mitigation approach).
...
Separately, we benchmark quantum code generation: frontier LLMs achieve 62--64\% on the Qiskit HumanEval suite zero-shot, 68--71\% with retrieval-augmented generation, and 79.5\% via 3-run ensemble---with 2.7 percentage point run-to-run variance even at temperature zero.
...
\begin{enumerate}
  ...
  \item A hybrid post-selection + readout error mitigation (PS+REM) approach that achieves chemical accuracy on a 9-qubit platform, and an empirical mitigation technique ranking for shallow circuits.
  \item A Qiskit HumanEval code generation benchmark showing 68--71\% with RAG, 79.5\% ensemble ceiling, and 2.7pp run-to-run variance at temperature zero.
...
\noindent *Kim~\cite{kim2023} tested on emulator only (9-qubit kicked Ising subset).
...
Superscripts: $^a$TREX mitigation, $^b$post-selection, $^c$Torino only, $^d$PS+REM hybrid, $^e$emulator only.}
...
\multirow{4}{*}{Sagastizabal~\cite{sagastizabal2019}}
  ...
  & Chemical accuracy & \PASS & \PASS{} (0.92)$^d$ & \PASS{} (0.22)$^a$ & \PASS \\
...
\multirow{3}{*}{Kim~\cite{kim2023}}
  & Kicked Ising dynamics & \PASS$^e$ & --- & --- & \PASS \\
  & ZNE improves accuracy & \PASS$^e$ (14.1$\times$) & --- & --- & \PASS \\
  & 9-qubit scaling & \PASS$^e$ & --- & --- & \PASS \\
\midrule
\multicolumn{2}{l}{\textbf{Total}} & \textbf{26/26} & \textbf{21/23} & \textbf{17/19} & \textbf{25/27} \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
H$_2$ at equilibrium bond length ($R = 0.735$\,\AA) achieves chemical accuracy on two platforms: IBM Torino achieves $-1.138\Ha$ ($0.22\kcalmol$) with TREX error mitigation~\cite{kim2023}, and Tuna-9 achieves $0.92\kcalmol$ with a hybrid post-selection + readout error mitigation (PS+REM) approach on qubit pair q[2,4].
A second qubit pair q[6,8] also passes at $1.32\kcalmol$.
...
\subsection{Hybrid PS+REM on Tuna-9}

On Tuna-9, post-selection alone gives $\sim$7$\kcalmol$ mean error. Readout error mitigation (REM) via confusion matrix calibration and inversion corrects measurement bias in $X/Y$ bases, but fails catastrophically ($39\kcalmol$) when gate noise spikes. The hybrid approach applies post-selection to $Z$-basis measurements (catching parity leakage) and REM to $X/Y$-basis measurements (correcting readout bias), achieving $0.92\kcalmol$ on qubit pair q[2,4] and $1.32\kcalmol$ on q[6,8]---both within chemical accuracy.

\subsection{Kim et al.\ (2023): Utility-Scale Circuits}

We replicated the kicked Ising dynamics from Kim et al.~\cite{kim2023} on the 9-qubit emulator (a scaled-down version of the original 127-qubit experiment). The emulator reproduces the expected magnetization dynamics, and ZNE via gate folding provides a $14.1\times$ improvement in accuracy over noisy simulation---consistent with the paper's demonstration that ZNE enables meaningful computation beyond classical simulability thresholds.
...
Our H$_2$ replication succeeded on IBM Torino with TREX mitigation ($119\times$ error reduction) and on Tuna-9 with hybrid PS+REM ($0.92\kcalmol$).
...
Chemical accuracy was achieved for H$_2$ VQE on both IBM Torino ($0.22\kcalmol$ with TREX) and QI Tuna-9 ($0.92\kcalmol$ with hybrid PS+REM).
