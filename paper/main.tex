...
Extending to LiH (4 qubits, CASCI(2,2) active space), we observe a $3\times$ error increase over H$_2$, consistent with the additional CZ gate layers and quantifying the scaling limit for chemistry on current hardware.
...
The $3\times$ error increase from H$_2$ (7.4\,mHa with REM) to LiH (23.4\,mHa) is consistent with the additional CZ gates: the 2-qubit H$_2$ circuit has 1 CZ gate, while the 4-qubit LiH circuit has 3, each contributing $\sim$5--8\,mHa of accumulated gate error.
Calibration drift during the LiH batch was minimal ($< 1\%$ mean fidelity change across 16 calibration states).
This quantifies the practical scaling limit of Tuna-9 for chemistry: 2-qubit VQE achieves chemical accuracy with mitigation, but 4-qubit VQE requires error correction or substantially improved gate fidelities.
...
Extending to LiH (4 qubits, CASCI(2,2)) reveals a $3\times$ error increase over H$_2$ ($23.4$ vs.\ $7.4$\,mHa with REM), consistent with the tripling of CZ gate layers and establishing the practical boundary where error mitigation alone is insufficient.
