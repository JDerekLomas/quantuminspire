...
<p>The most cited quantum computing paper of 2023 demonstrated 127-qubit Trotterized time evolution on IBM's heavy-hex lattice with PEA (Probabilistic Error Amplification) mitigation. We replicated the core physics on 9 qubits using the Tuna-9 topology with basic ZNE (gate folding + Richardson extrapolation) &mdash; first on emulator with simulated noise, then on IBM hardware with a 5-qubit chain, and finally on <strong>Tuna-9 hardware using all 9 qubits and 10 edges</strong>.</p>

<p>Three claims tested, all pass:</p>

<ol>
<li><strong>M_z decays with depth</strong> &mdash; Emulator: 0.972 (d=1) to 0.755 (d=10). IBM: 0.92-0.96 at Clifford. <strong>Tuna-9 hardware: 0.944 (d=1) &rarr; 0.785 (d=3) &rarr; 0.585 (d=5)</strong>. Per-qubit analysis reveals dramatic position-dependent error: q0 (edge qubit) maintains Z=0.951 at d=5 while q8 (chain end) collapses to Z=0.025.</li>
<li><strong>ZNE recovers ideal at Clifford</strong> &mdash; Emulator: 24.5% &rarr; 3.0% at d=10. IBM Marrakesh: 3.2% &rarr; 1.0%. <strong>Tuna-9: d=1 noisy 0.944 &rarr; ZNE 0.982 (1.8% error, 3.1x improvement). d=3: 0.785 &rarr; ZNE 0.857 (1.5x)</strong>. At d=3, fold=3 requires 180 CZ gates &mdash; hardware decoherence saturates.</li>
<li><strong>ZNE improves over unmitigated</strong> &mdash; Emulator: <strong>14.1x</strong>. IBM: <strong>3.1x</strong>. <strong>Tuna-9: 3.1x (d=1), 1.5x (d=3), mean 2.3x</strong>. All three hardware backends show 2-3x improvement from basic ZNE gate folding &mdash; well below the emulator's 14.1x because real noise includes dephasing and coherent errors that simple gate folding cannot linearly extrapolate away.</li>
</ol>

<p>Noise amplification on IBM is clean: P(|00000&rang;) drops 92.8% &rarr; 84.1% &rarr; 70.1% (fold 1,3,5). On Tuna-9, the same pattern holds at larger scale: P(all-zero) drops 79.3% (d=1,f=1) &rarr; 56.3% (d=1,f=3) and 42.4% (d=3,f=1) &rarr; 18.7% (d=3,f=3). Gate folding amplifies noise monotonically on both platforms.</p>
...
<p>VQE mitigation data: <a href="https://github.com/JDerekLomas/quantuminspire/blob/main/experiments/results/vqe-mitigation-ladder-001-ibm.json">IBM mitigation ladder</a>. ZNE gate-folding data: <a href="https://github.com/JDerekLomas/quantuminspire/blob/main/experiments/results/">experiments/results/</a>. Kim 2023 replication: <a href="https://github.com/JDerekLomas/quantuminspire/blob/main/experiments/results/kim2023-ising-tuna9.json">kicked Ising results</a>.</p>`,
    sources: [
      ...
      { label: 'Error mitigation showdown', url: '/blog/error-mitigation-showdown' },
      { label: 'Kim et al., Nature 618, 2023', url: 'https://doi.org/10.1038/s41586-023-06096-3' },
